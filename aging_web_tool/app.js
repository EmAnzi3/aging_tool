...
async function generateReports() {
  ...
  if (!clientiFile || !filialeFile || !mesiFile) {
    setStatus('Carica tutti e tre i file prima di generare i report.', 'error');
    return;
  }
  ...
  try {
    const [clientiData, filialeData, mesiData] = await Promise.all([
      parseXlsxFile(clientiFile),
      parseXlsxFile(filialeFile),
      parseXlsxFile(mesiFile),
    ]);
    ...
    state.outputs = {
      agingClienti: { filename: 'Aging Clienti.xlsx', data: agingClientiXlsx },
      agingFiliale: { filename: 'Aging Filiale.xlsx', data: agingFilialeXlsx },
      dormienti: { filename: 'Clienti Dormienti e Persi.xlsx', data: dormientiXlsx },
    };
    ...
    if (warnings.length) {
      ...
      setStatus('Report generati correttamente. Puoi scaricare il pacchetto ZIP o i singoli file.', 'ok');
    }
  } catch (err) {
    console.error(err);
...
async function parseStyles(zip) {
  ...
  const fonts = fontNodes.map((font) => font.getElementsByTagName('b').length > 0);
  const cellXfs = doc.getElementsByTagName('cellXfs')[0];
  const xfNodes = cellXfs ? Array.from(cellXfs.getElementsByTagName('xf')) : [];
